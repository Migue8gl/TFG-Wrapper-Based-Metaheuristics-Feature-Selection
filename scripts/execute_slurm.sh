...
# Define the directory to store error messages
...
for opt in "${optimizers[@]}"; do
    for dataset in "${datasets[@]}"; do
        ...
        sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=${opt}_${dataset_name}
#SBATCH --output=${log_file}
#SBATCH --error=${log_file}

python3 src/main.py -o "$opt" -d "$dataset" ${1+"$@"} 
EOT
        ...
        if [[ "$opt" != "aco" ]]; then
            ...
            sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=${opt}_real_${dataset_name}
#SBATCH --output=${log_file}
#SBATCH --error=${log_file}

python3 src/main.py -o "$opt" -b 'r' -d "$dataset" ${1+"$@"} 
EOT
        fi
    done
done
