...
\subsection{Representación y Operadores}
La representación suele ser la misma que en otros algoritmos metaheurísticos. Una matriz de $N\times M$, donde $N$ es el número de filas o número de invididuos en la población y $M$ es el número de características del problema. DE fue diseñado para optimizar un problema continuo, por lo que la población debería ser un conjunto de números reales.

\subsubsection{Mutación}
La operación de mutación produce un vector, conocido como vector de prueba o en inglés \textit{trial vector}. Para ello se hace uso de un vector objetivo y una diferencia de vectores ponderada. La mutación elige a un padre $x_{i}$, generando un vector de prueba $u_{i}$ sigueindo lo siguientes pasos:
\begin{enumerate}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[Mutación en DE]{Esta figura ha sido seleccionada de una clase dada por Jaco van Niekerk. Muestra de manera gráfica en un espacio bidimensional como funciona el operador de cruce al recombinar los genes/características de los vectores padre y ``trial''.}
\end{figure}
...
\section{Optimización por enjambre de abejas artificial}
\subsection{Introducción}
...
\subsection{Representación y Operadores}
La representación en este algoritmo es exactamente igual que en el resto de algoritmos evolutivos, vectores que representan soluciones y que están formado por una serie de elementos, tantos de ellos como características tenga el problema. En cuanto a los operadores del algoritmo, exiten los siguientes:
\begin{enumerate}
