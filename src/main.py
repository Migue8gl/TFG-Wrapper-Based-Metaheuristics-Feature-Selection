...
def main(*args, **kwargs):
    ...
    encoding = (
        "real"
        if (
            "binary" in optimizer.params
            and (optimizer.params["binary"] == "r" or not optimizer.params["binary"])
        )
        else "binary"
    )
    ...
    metrics_svc = evaluate_optimizer(
    ...
    file_path = os.path.join(
        RESULTS_DIR, encoding, dataset_name, "all_fitness_svc.json"
    )
    ...
    metrics_knn = evaluate_optimizer(
        dataset=dataset_dict,
        optimizer=optimizer,
        n=i,
        scaler=scaling_arg,
    ...
    file_path = os.path.join(
        RESULTS_DIR, encoding, dataset_name, "all_fitness_knn.json"
    )
    ...
    data = {
        ...
        "avg": [metrics_knn["test_fitness"]["avg"], metrics_svc["test_fitness"]["avg"]],
        "std_dev": [
    ...
    df.to_csv(
        result_path + "/{}_{}.csv".format(dataset_name, optimizer_arg), index=True
    )
    ...
    plot_metric_over_folds(
        ...
        "orange",
        ax=None,
        title="Average selected features over {} evaluations running {} (SVC)".format(
            i, optimizer_arg
        ),
    )
    ...
    plot_metric_over_folds(
        ...
        "orange",
        ax=None,
        title="Average selected features over {} evaluations running {} (KNN)".format(
            i, optimizer_arg
        ),
    )
    ...
    if notify_arg:
        ...
        notifications.send_telegram_message(
            token=token,
            chat_id=chat_id,
            message="### Execution finished - Total time {} seconds ###".format(
                round(total_time, 4)
            ),
        )
