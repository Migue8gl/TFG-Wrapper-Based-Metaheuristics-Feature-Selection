...
def main(*args, **kwargs):
    ...
    for opt in optimizer_dict.keys():
       ...
       # Print average accuracy over k folds
        ...
        plot_fitness_over_folds(
            fitness_values, parameters[second_key], k, ax=ax1, title='Average fitness {}-fold cross validation (SVC)'.format(k))
        columns += 1
        ax2 = fig.add_subplot(gs[rows, columns])
        plot_fitness_over_population_sizes(total_fitness_test, np.arange(
            5, 60, 10), ax=ax2, title='Fitness test value over population sizes (SVC)')
        ...
        plot_fitness_over_folds(
            fitness_values_2, parameters[second_key], k, ax=ax3, title='Average fitness {}-fold cross validation (KNN)'.format(k))
        columns += 1
        ax4 = fig.add_subplot(gs[rows, columns])
        plot_fitness_over_population_sizes(total_fitness_test_2, np.arange(
            5, 60, 10), ax=ax4, title='Fitness test value over population sizes (KNN)')
        rows += 1
    ...
    if (notify_arg):
        ...
        notifications.send_telegram_image(
            image_path='./images/dashboard.jpg', caption='-- Dashboard de la ejecución --')
