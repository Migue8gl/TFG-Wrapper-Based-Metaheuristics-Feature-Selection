...
for (metric in metrics) {
    write.csv(pairwise.wilcox.test(df_binary[[metric]], df_binary$optimizer, p.adjust.method = "holm")$p.value,
        file = paste0("results/stats/wilcox_test_", metric, "_binary.csv")
    )
    write.csv(pairwise.wilcox.test(df_real[[metric]], df_real$optimizer, p.adjust.method = "holm")$p.value,
