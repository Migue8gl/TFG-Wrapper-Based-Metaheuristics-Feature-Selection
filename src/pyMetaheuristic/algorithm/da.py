...
def update_position(a, c, f, e, s, w, r, beta, sigma, enemy_pos, food_pos, delta_max, dragonflies, deltaflies, min_values, max_values, target_function, target_function_parameters, binary):
    for i in range(dragonflies.shape[0]):
        ...
        A      =  np.mean(neighbours_delta,  axis = 0) if neighbours_delta else deltaflies[i, :-2]
        C      =  np.mean(neighbours_dragon, axis = 0) - dragonflies[i, :-2] if neighbours_dragon else np.zeros(len(min_values))
        S      = -np.sum (neighbours_dragon - dragonflies[i, :-2], axis = 0) if neighbours_dragon else np.zeros(len(min_values))
        dist_f = euclidean_distance(dragonflies[i, :-2], food_pos [0, :-2])
        dist_e = euclidean_distance(dragonflies[i, :-2], enemy_pos[0, :-2])
        F      = food_pos [0, :-2]- dragonflies[i, :-2] if (dist_f <= r).all() else np.zeros(len(min_values))
        E      = enemy_pos[0, :-2] if (dist_e <= r).all() else np.zeros(len(min_values))
        for k in range(len(min_values)):
            if ( (dist_f > r).all() ):
                if len(neighbours_dragon) > 1:
                    deltaflies[i, k] = w * deltaflies[i, k] + np.random.rand() * (a * A[k] + c * C[k] + s * S[k])
                else:
                    dragonflies[i, :-2] = dragonflies[i, :-2] + levy_flight(len(min_values), beta, sigma) * dragonflies[i, :-2]
...
def dragonfly_algorithm(size = 3, min_values = [-5,-5], max_values = [5,5], generations = 50, target_function = target_function, verbose = True, binary = 'x', target_function_parameters = None):
    ...
    food_pos    = initial_variables(1, min_values, max_values, target_function, target_function_parameters)
    enemy_pos   = initial_variables(1, min_values, max_values, target_function, target_function_parameters)
    dragonflies = initial_variables(size, min_values, max_values, target_function, target_function_parameters) 
    deltaflies  = initial_variables(size, min_values, max_values, target_function, target_function_parameters) 
    beta        = 3/2
