        ...
        for j in range(0, len(min_values)):
            if (p < 0.5):
                if (abs(a_leader) >= 1):
                    ...
                    if binary == 's':
                        position[i, j] = s_shaped_transfer_function(
                            np.clip(leader[0, j] - a_leader*distance_leader, min_values[j],  max_values[j]))
                    elif binary == 'v':
                        position[i, j] = v_shaped_transfer_function(
                            np.clip(leader[0, j] - a_leader*distance_leader, min_values[j],  max_values[j]))
                    else:
                ...
                if binary == 's':
                    position[i, j] = s_shaped_transfer_function(np.clip((distance_Leader*math.exp(
                        spiral_param*m_param)*math.cos(m_param*2*math.pi) + leader[0, j]), min_values[j],  max_values[j]))
                elif binary == 'v':
                    position[i, j] = v_shaped_transfer_function(np.clip((distance_Leader*math.exp(
                        spiral_param*m_param)*math.cos(m_param*2*math.pi) + leader[0, j]), min_values[j],  max_values[j]))
                else:
...
def whale_optimization_algorithm(hunting_party=5, spiral_param=1,  min_values=[-5, -5], max_values=[5, 5], iterations=50, target_function=target_function, verbose=True, target_function_parameters=None, binary='x'):
    ...
    position = initial_position(
        hunting_party, min_values, max_values, target_function, target_function_parameters)
    leader = leader_position(dimension=len(min_values), min_values=min_values, max_values=max_values,
    ...
    while (count <= iterations):
        ...
        position = update_position(position, leader, a_linear_component, b_linear_component,
                                   spiral_param, min_values, max_values, target_function, target_function_parameters, binary)
        count = count + 1
        fitness_values.append(
            {'ValFitness': leader[0, -1], 'TrainFitness': leader[0, -2]})
